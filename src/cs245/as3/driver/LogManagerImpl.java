...
public class LogManagerImpl implements LogManager {
    ...
    private int Riop_counter;  //已读取次数
    private int Wiop_counter;  //已写入次数
    ...
    public byte[] readLogRecord(int position, int size) throws ArrayIndexOutOfBoundsException {
        ...
        if (position < logTruncationOffset || position + size > getLogEndOffset()) {
            throw new ArrayIndexOutOfBoundsException("Offset " + (position + size) + "invalid: log start offset is " +
                    logTruncationOffset + ", log end offset is " + getLogEndOffset());
        }
